...
2026-09-23 起網站程式只讀取 `.env`、`.env.apify` 及程序環境變數，不再呼叫 macOS `security` 或讀取 Chrome 金鑰。這是憑證存放位置的遷移，不是輪替金鑰；既有登入、私密訂閱網址與貢獻 token 應維持可用。歷史稽核文件中的 Keychain 待辦反映當時狀態，目前操作以本頁為準。
