...
Linux 需使用 `playwright install --with-deps chromium`。Caddy 路由測試另需安裝 Caddy 2.11.2（與正式機一致）；GitHub Actions 從官方 release 安裝並核對 repository 內 SHA-512，因此不會略過。Ubuntu 24.04 的預設 apt 套件太舊，無法解析 `handle_errors 404`。CI 的 `verify` job 應設為主分支保護的必要檢查；workflow 本身不部署，也不變更 repository 分支保護設定。
...
更新後應重新建立乾淨虛擬環境執行以上安裝與檢查。Playwright 的 browser revision 隨 lock 內套件版本固定；Node/Python 的 patch 與作業系統安全更新由 CI runner 提供，並非 bit-for-bit 系統映像。
