...
(function () {
  ...
  (function () {
    ...
    // 桌機側欄：依序插在「更多」前面
    // 首頁殼層已靜態放了同一組項目（避免載入時側欄跳動）：先清掉再照清單重建，內容相同不會跳
    nav.querySelectorAll(".nav-desktop-extra").forEach(function (a) { a.remove(); });
  ...
  (function () {
    ...
    document.addEventListener("click", function (e) {
      document.querySelectorAll("details.feed-filters[open], details.nav-more[open], details.col-picker[open], details.addcol[open]").forEach(function (d) {
  ...
  (function () {
    ...
    // 殼層裡已有靜態按鈕（首頁）就直接接手，沒有才建
    var btn = header.querySelector(".topbar-search");
    if (!btn) {
    ...
    if (nav) navSearch = nav.querySelector(".nav-search");
    if (nav && !navSearch) {
  ...
  function initFeed() {
    ...
    if (!feed) return;
    // 貼文右上角的平台標誌（Simple Icons）；公告頁與官方 API 用訂閱源符號
    var PLAT_ICON = {
    ...
    PLAT_ICON.api = PLAT_ICON.bulletin;
    // 拉丁字前留一個空格：「陽明游泳社的 Instagram 主頁」
    var PLAT_PAGE = { instagram: " Instagram 主頁", facebook: " Facebook 粉專", threads: " Threads 主頁", x: " X 主頁", bulletin: "公告頁", api: "官方網站" };
    ...
    fetch("/data/posts.json").then(function (r) { return r.json(); }).then(function (data) {
      ...
      // 右上角的平台標誌：看出來源是哪個平台，點了到來源帳號主頁（原文連結在下方動作列）
      function platBadge(p) {
        var icon = PLAT_ICON[p.platform];
        if (!icon) return "";
        var page = PLAT_PAGE[p.platform] || p.platform;
        if (!p.profile_url) return '<span class="feed-plat" title="' + esc(page.trim()) + '">' + icon + "</span>";
        var label = esc((p.source_name || "") + "的" + page);
        return '<a class="feed-plat" href="' + esc(p.profile_url) + '" target="_blank" rel="noopener" title="' + label + '" aria-label="' + label + '">' + icon + "</a>";
      }
      ...
      function row(p) {
        ...
        var body = (p.text ? '<p class="feed-text' + (longText ? " is-long" : "") + '">' + esc(p.text) + "</p>" : "") +
          ...
          (p.image ? '<img class="feed-img" src="' + esc(p.image) + '" alt=""' +
            (p.image_w && p.image_h ? ' width="' + p.image_w + '" height="' + p.image_h + '"' : "") + ' loading="lazy">' : "");
  ...
  function initList(bundle) {
    ...
    var chipGroups = {};
    buildChips("f-sort", [["time", "時間"], ["going", "熱門"]], "sort");
    buildChips("f-time", [
    ...
    function render() {
      ...
      if (state.sort === "going") {
        ...
        list = list.slice().sort(function (a, b) {
          ...
          return cb - ca || new Date(a.start_at) - new Date(b.start_at);
        });
...
(function () {
  ...
  function apply(data) {
    ...
    counts = data.counts || {};
    mine = {};
    (data.going || []).forEach(function (id) { mine[id] = true; });
    ready = true;
    refresh();
    window.dispatchEvent(new CustomEvent("chumei-going-change"));
  }
  // 只在值真的變了才寫 DOM：寫 textContent 會產生 childList 變動，
  // 而下面的 MutationObserver 正是觀察 childList——無條件寫入會自我觸發成無限迴圈。
  ...
  function load() {
    return fetch("/auth/events", { credentials: "same-origin" })
      .then(function (r) { return r.json(); }).then(apply).catch(function () {});
  }
  document.addEventListener("click", function (ev) {
    var b = ev.target.closest(".going-btn[data-event-id]");
    if (!b) return;
    ev.preventDefault();
    toggle(b.dataset.eventId, b);
  });
  new MutationObserver(refresh).observe(document.documentElement, { childList: true, subtree: true });
  window.chumeiGoing = {
    count: function (id) { return n(counts[id]); },
    isGoing: function (id) { return !!mine[id]; },
