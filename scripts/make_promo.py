...
輸出：state/promo/out/
  hero-4x5.png       2160×2700  Threads/IG 主圖（首頁截圖手機框＋數字）
  story-9x16.png     2160×3840  IG/FB 限時動態
  hero-1x1.png       2160×2160  Dcard/FB 縮圖
  slide-{1..4}.png   2160×2700  輪播：功能介紹四張
  cover-16x9.png     2400×1260  Dcard/連結預覽
...
from make_brand import BG, WHITE, MUTED, GRAD_CHU, GRAD_MEI, font as brand_font, draw_wordmark, _hex, _lerp
...
def font(size):
    """字體直接以輸出解析度繪製，避免先畫小圖再放大。"""
    return brand_font(u(size))
...
def phone(shot_name, width, crop_h=None, crop_top=0):
    ...
    d.rounded_rectangle((0, 0, W - 1, H - 1), radius=r, fill="#141622", outline="#2A2E42", width=u(3))
...
def wordmark_line(im, cx, cy, size):
    ...
    wm_w = round(u(size) * 2.06)
    gap = round(u(size) * 0.12)
    total = wm_w + gap + rest_w
    x0 = cx - total / 2
    draw_wordmark(im, round(x0 + wm_w / 2), cy, u(size))
...
def stat_row(im, y, items, size=64, label_size=28):
    ...
    for i, (num, label, grad) in enumerate(items):
        ...
        fl = font(label_size)
        d.text((cx - d.textlength(label, font=fl) / 2, y + u(size + 14)), label, font=fl, fill=MUTED)


def hero(W, H, s, top_offset=0):
    im = Image.new("RGB", (W, H), BG)
    glow(im, round(W * 0.2), round(H * 0.15), u(420), GRAD_CHU[0], 70)
    glow(im, round(W * 0.85), round(H * 0.35), u(420), GRAD_MEI[0], 60)
    tall = H > W
    wordmark_line(im, W // 2, u((130 if tall else 100) + top_offset), 84 if tall else 72)
    y = u((230 if tall else 180) + top_offset)
    text_center(im, y, "陽明/交大、清大", 70 if tall else 58, WHITE)
    text_center(im, y + u(94 if tall else 78), "校園活動資訊一站看完", 70 if tall else 58, WHITE)
    text_center(im, y + u(215 if tall else 180),
    ...
    stat_row(im, y + u(290 if tall else 240), items, 68 if tall else 56, 28 if tall else 24)
    pw = u(560 if tall else 470)
    py = y + u(460 if tall else 370)
    # 保留正常的長手機比例；超出畫布的內容由畫布邊緣漸隱，而不是把手機
    # 本身裁成一個矮矮的圓角框。
    ph = phone("home", pw)
    paste_phone_fade(im, ph, (W - ph.width) // 2, py,
                     fade=u(200), visible_h=H - py)
    text_center(im, H - u(92), "https://竹梅.tw", 32, MUTED)
    return im


def slide(title, sub, shot_name, W=None, H=None, grad=GRAD_CHU, crop_top=0, index=None):
    W = W or u(1080)
    H = H or u(1350)
    im = Image.new("RGB", (W, H), BG)
    glow(im, W // 2, round(H * 0.1), u(460), grad[0], 60)
    d = ImageDraw.Draw(im)
    if index:
        d.text((u(72), u(64)), index, font=font(30), fill=MUTED)
    d.text((u(72), u(120)), title, font=font(72), fill=WHITE)
    f = font(32)
    yy = u(230)
    for line in sub.split("\n"):
        d.text((u(72), yy), line, font=f, fill=MUTED)
        yy += u(48)
    ph = phone(shot_name, u(620), crop_top=u(crop_top))
    paste_phone_fade(im, ph, (W - ph.width) // 2, u(380),
                     fade=u(200), visible_h=H - u(380))
    wordmark_line(im, W // 2, H - u(80), 36)
    return im
...
def cover(s):
    W, H = u(1200), u(630)
    im = Image.new("RGB", (W, H), BG)
    glow(im, u(200), u(100), u(380), GRAD_CHU[0], 70)
    glow(im, u(1000), u(550), u(380), GRAD_MEI[0], 60)
    d = ImageDraw.Draw(im)
    draw_wordmark(im, u(360), u(200), u(180))
    cover_label = "活動觀測站"
    cover_label_font = font(72)
    d.text((u(360) - d.textlength(cover_label, font=cover_label_font) / 2, u(280)),
           cover_label, font=cover_label_font, fill=WHITE)
    d.text((u(90), u(405)), "陽明/交大、清大校園活動資訊一站看完", font=font(30), fill=MUTED)
    ph = phone("home", u(400))
    paste_phone_fade(im, ph, u(720), u(50), fade=u(90), visible_h=H - u(50))
    domain = "https://竹梅.tw"
    domain_font = font(24)
    d.text((u(360) - d.textlength(domain, font=domain_font) / 2, u(505)),
...
def main():
    ...
    s = stats()
    print(s)
    hero(u(1080), u(1350), s).save(OUT / "hero-4x5.png")
    hero(u(1080), u(1080), s).save(OUT / "hero-1x1.png")
    hero(u(1080), u(1920), s, top_offset=110).save(OUT / "story-9x16.png")
