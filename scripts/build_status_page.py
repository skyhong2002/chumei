...
(() => {
  ...
  function renderRows(){const q=state.filter.toLowerCase();const rows=state.data.sources.filter(s=>(!q||`${s.name} ${s.username} ${s.backend}`.toLowerCase().includes(q))&&(!state.platform||s.platform===state.platform)&&(!state.status||s.status===state.status));document.querySelector('#source-count').textContent=`顯示 ${rows.length.toLocaleString()} / ${state.data.sources.length.toLocaleString()} 筆`;
    document.querySelector('#source-rows').innerHTML=rows.map(s=>`<tr><td><div class="status-source-name">${esc(s.name)}</div><div class="status-source-sub">${esc(s.platform)} ${esc(s.kindLabel||'')} · ${esc(s.username)}</div></td><td>${esc(s.backend)}</td><td><span class="pill ${s.status}">${s.status==='ok'?'正常':s.status==='due'?'等待排程':'錯誤'}</span>${s.lastError?`<div class="error-text">${esc(s.lastError)}</div>`:''}</td><td class="mono">${fmtTime(s.lastAttempt)}</td><td class="mono">${fmtTime(s.lastSuccess)}</td><td class="mono">${fmtTime(s.nextDue)}</td><td>${fmtInterval(s.targetIntervalHours)}<div class="status-source-sub">實際平均：${fmtInterval(s.averageIntervalHours)}</div></td><td><button class="fetch-button" data-id="${esc(s.id)}" ${!state.auth||s.blockedReason?'disabled':''} title="${esc(!state.auth?'登入後可要求優先抓取':s.blockedReason||'加入優先抓取佇列')}">${s.blockedReason?'額度受限':'優先抓取'}</button></td></tr>`).join('')||'<tr><td colspan="8">沒有符合的來源。</td></tr>';}
...
<section class="status-page">
  ...
  <div class="source-detail-head"><h2>各來源抓取明細</h2><p>同一個 Instagram 帳號會分成「貼文」與「限時動態」兩列，因為它們使用不同抓法與排程。</p></div>
  ...
  <div class="status-table-wrap"><table class="status-source-table"><thead><tr><th>來源／內容</th><th>爬取方式</th><th>狀態</th><th>最後嘗試</th><th>最後成功</th><th>下次預定</th><th>頻率</th><th>操作</th></tr></thead><tbody id="source-rows"><tr><td colspan="8">載入中…</td></tr></tbody></table></div>
